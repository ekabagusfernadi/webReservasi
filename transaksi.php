<?php
    ...
    if( isset($_POST["submit"]) ) {
        ...
        $queryCekNamaTransaksi = "SELECT id_transaksi, nama_menu_saat_ini, jumlah_pesanan FROM transaksi WHERE kode_reservasi = '$id'";
        $objekCekNamaTransaksi = mysqli_query($conn, $queryCekNamaTransaksi);
        $namaMenuSaatInis = [];
        while( $namaMenuSaatIni = mysqli_fetch_assoc($objekCekNamaTransaksi) ) {
            $namaMenuSaatInis[] = $namaMenuSaatIni;
        }


        foreach( $namaMenuSaatInis as $namaMenuSI ) {
            if( $namaMenuSI["nama_menu_saat_ini"] == $namaMenu) {
                $jumlahPesananDB = $namaMenuSI["jumlah_pesanan"];
                $totalJumlahMenu = $jumlahPesananDB + $jumlahPesanan;
                $idTransaksi = $namaMenuSI["id_transaksi"];
                $queryUpdateTransaksi = "UPDATE transaksi SET jumlah_pesanan = '$totalJumlahMenu' WHERE id_transaksi = '$idTransaksi'";
...
<body>
    <header id="header">
        <div class="container">
            <center>
                <img src="kijangWarteg.png" width="70" class="logo">
